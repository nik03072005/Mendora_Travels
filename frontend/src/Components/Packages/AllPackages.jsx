...
const AllPackageCards = ({ title, packages, viewAllRoute }) => {
  ...
  return (
    <section className="px-4 py-4 w-full mx-auto max-w-[1140px] relative">
  ...
  {convertedPackages?.map((pkg) => (
    ...
    >
      <img
        src={pkg.image}
        alt={pkg.title}
        className="w-full h-48 sm:h-56 md:h-64 object-cover cursor-pointer rounded-2xl"
        onClick={() => handleCardClick(pkg)}
      />
      <div className="pt-4 space-y-2">
        <p className="text-sm text-gray-500">{pkg.nights}</p>
        <h3 className="font-semibold text-lg cursor-pointer" onClick={() => handleCardClick(pkg)}>{pkg.title}</h3>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-lg w-80 text-center relative">
            <button
              ...
              className="absolute top-2 right-3 cursor-pointer text-gray-500 hover:text-black"
            >
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
    <div className="bg-white p-4 sm:p-5 rounded-lg shadow-lg max-w-md w-[95%] sm:w-[90%] max-h-[90vh] overflow-y-auto relative">
      <button
        onClick={closeDialogs}
        className="absolute top-2  cursor-pointer right-2 text-gray-500 hover:text-black text-sm sm:text-base"
      >
        <FontAwesomeIcon icon={faTimes} />
      </button>
...
export default AllPackageCards;