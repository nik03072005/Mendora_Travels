    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        {highlights && highlights.length > 0 && (
          <div className="flex gap-2 mb-4 overflow-hidden">
            {highlights.slice(0, 3).map((highlight, index) => (
              <span 
                key={index}
                className="text-xs bg-pink-50 text-pink-700 px-2 py-1 rounded-full whitespace-nowrap truncate"
                title={highlight}
              >
                {highlight}
              </span>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          ...
          <button className="bg-pink-600 hover:bg-pink-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors mt-auto">
            View Details
          </button>
        </div>
      </div>
