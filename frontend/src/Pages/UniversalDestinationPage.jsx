...
const UniversalDestinationPage = () => {
  ...
  // Fetch destination data from API using slug
  useEffect(() => {
    const fetchPageData = async () => {
      try {
        ...
        console.error(`Error fetching ${slug} page data:`, err);
  ...
  // Using fallback default values (like Kashmir page)
  const destination = pageData?.destination || {
    destinationName: slug?.charAt(0).toUpperCase() + slug?.slice(1) || 'Destination',
    heroSection: {
      title: `${slug?.charAt(0).toUpperCase() + slug?.slice(1)} Tour Packages`,
    ...
    aboutSection: {
      shortDescription: 'Explore this amazing destination with our carefully curated tour packages.',
      expandedDescription: 'Experience unforgettable moments and create lasting memories with our expertly designed tours.'
    },
    activities: [],
    groupTours: [],
    subDestinations: [],
  ...
  return (
    <>
      <Helmet>
        <title>{destination.heroSection?.title || `${destination.destinationName} Tour Packages`} | Mendora Travels</title>
        <meta name="description" content={destination.aboutSection?.shortDescription || `Explore ${destination.destinationName} with our curated tour packages.`} />
      </Helmet>
      ...
      <section className="py-16 bg-blue-600 text-white">
        <div className="container mx-auto px-4">
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">Plan Your {destination.destinationName} Trip</h2>
            <p className="mb-8">Get in touch with our travel experts to customize your perfect {destination.destinationName} adventure</p>
            <form onSubmit={handleFormSubmit} className="bg-white rounded-lg p-8 text-gray-800">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <input
                  ...
                  className="px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-blue-600"
                  required
                />
                <input
                  type="email"
              ...
              <textarea
                name="message"
                placeholder={`Tell us about your dream ${destination.destinationName} trip...`}
                onChange={handleFormChange}
                rows="4"
                className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-blue-600 mb-4"
                required
...
export default UniversalDestinationPage;
