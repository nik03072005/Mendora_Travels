...
const BaliTourPackagesPage = () => {
    ...
    const fetchPageData = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/destinations/page/bali`
        );
        setPageData(response.data);
        setError(null);
      } catch (err) {
        console.error('Error fetching Bali page data:', err);
      ...
      <>
        ...
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading Bali tour packages...</p>
          </div>
        </div>
      </>
    );
  ...
  const destination = pageData?.destination || {
    ...
    aboutSection: {
      shortDescription: 'Bali, known as the Land of Smiles, is a perfect blend of ancient temples, modern cities, pristine beaches, and vibrant culture.',
      expandedDescription: 'Bali, known as the Land of Smiles, is a perfect blend of ancient temples, modern cities, pristine beaches, and vibrant culture. Experience the grandeur of Bangkok\'s temples, the excitement of Pattaya\'s beaches, the beauty of Phuket\'s islands, and the adventure of Krabi\'s landscapes.'
    },
    activities: [],
    groupTours: [],
    subDestinations: [],
  ...
  return (
    <>
      <Helmet>
        <title>{destination.heroSection?.title || 'Bali Tour Packages'} | Mendora Travels</title>
        <meta name="description" content={destination.aboutSection?.shortDescription || 'Explore Bali with our curated tour packages.'} />
      </Helmet>
